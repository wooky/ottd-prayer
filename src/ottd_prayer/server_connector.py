...
async def connect_to_server(config: Config, loop: asyncio.AbstractEventLoop, remote_server: RemoteServer) -> None:
    while True:
        reconnect_count = 1
        ...
        while True:
            try:
                logger.info(
                    "Attempt %d to connect to remote server", reconnect_count)
            ...
            reconnect_count += 1
            if not config.bot.auto_reconnect or reconnect_count > config.bot.reconnect_count:
...
async def _sleep(config: Config) -> None:
    logger.info("Waiting for %d seconds before retrying",
                config.bot.auto_reconnect_wait)
